...
function HarvestCore.client_onInteract( self, user, state )
	...
	if recipe and player then
		...
		if sm.container.canCollect( player:getInventory(), recipe.itemId, RA_MOD_REFINE_LOOT_NUM ) then
...
function HarvestCore.sv_refine( self, player )
	...
	if sm.exists( self.shape ) then
		...
		if recipe then
			...
			sm.container.collect( player:getInventory(), recipe.itemId, RA_MOD_REFINE_LOOT_NUM )			
		end
