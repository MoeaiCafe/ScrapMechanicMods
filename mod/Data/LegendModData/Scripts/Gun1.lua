...
Gun1 = class()
Gun1.maxParentCount = 2
Gun1.maxChildCount = 0
Gun1.connectionInput = bit.bor( sm.interactable.connectionType.logic, sm.interactable.connectionType.ammo )
Gun1.connectionOutput = sm.interactable.connectionType.none
Gun1.colorNormal = sm.color.new( 0xcb0a00ff )
Gun1.colorHighlight = sm.color.new( 0xee0a00ff )
Gun1.poseWeightCount = 1

local FireDelay = 5 --ticks
...
function Gun1.server_onCreate( self )
	self:sv_init()
end

-- (Event) Called when script is refreshed (in [-dev])
function Gun1.server_onRefresh( self )
	self:sv_init()
end

-- Initialize mounted gun
function Gun1.sv_init( self )
	self.sv = {}
...
function Gun1.server_onFixedUpdate( self, timeStep )
...
function Gun1.client_onCreate( self )
	self.cl = {}
...
function Gun1.client_onUpdate( self, dt )
...
function Gun1.client_getAvailableParentConnectionCount( self, connectionType )
