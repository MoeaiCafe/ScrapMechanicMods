...
JuPao3 = class( nil )
JuPao3.maxParentCount = 1
JuPao3.maxChildCount = 0
JuPao3.connectionInput = sm.interactable.connectionType.logic
JuPao3.connectionOutput = sm.interactable.connectionType.none
JuPao3.colorNormal = sm.color.new( 0xcb0a00ff )
JuPao3.colorHighlight = sm.color.new( 0xee0a00ff )
JuPao3.poseWeightCount = 1
JuPao3.fireDelay = 160 --ticks
JuPao3.minForce = 300
JuPao3.maxForce = 300
JuPao3.spreadDeg = 0.3
JuPao3.recoilForce = 0.05	--后坐力倍数
JuPao3.destructionRadius = 5.0	--破坏半径
JuPao3.shellMass = 35.0		--炮弹质量

function JuPao3.server_onCreate( self ) 
	self:server_init()
end

function JuPao3.server_init( self ) 
	self.fireDelayProgress = 0
	self.canFire = true
	self.parentActive = false
end

function JuPao3.server_onRefresh( self )
	self:server_init()
end

function JuPao3.server_onFixedUpdate( self, timeStep )
		...
		if bullet then
			if bullet.hit or bullet.alive > 200 then --lives for 2 sec, clean up after
				if bullet.hit then
					...
					sm.physics.explode( bullet.result, 8, self.destructionRadius, 12 , 40,"HangBaoDan")
				end
				self.bullets[k] = nil
			end
		end
...
function JuPao3.server_tryFire( self )
	local parent = self.interactable:getSingleParent()
	if parent then
		if parent:isActive() and not self.parentActive and self.canFire then
			...
			local offSetPos = (self.shape.right * 0) + (self.shape.up * 4.4) + (self.shape.at * 0.0)
...
function JuPao3.client_onCreate( self )
	self.boltValue = 0.0
	self.bullets = {}
...
function JuPao3.client_onFixedUpdate( self, dt )
...
function JuPao3.client_onShoot( self, data )
	self.boltValue = 1.0
	local rot = sm.vec3.getRotation( sm.vec3.new( 1, 0, 0 ), self.shape.up )

	local position = self.shape.worldPosition + self.shape.up*0.2
	local bullet = {effect = sm.effect.createEffect("JuPao3Shell"), pos = position, direction = data.dir, alive = 0, grav = data.gravity} -- -  self.shape.up*0.5
