...
JuPao1 = class( nil )
JuPao1.maxParentCount = 1
JuPao1.maxChildCount = 0
JuPao1.connectionInput = sm.interactable.connectionType.logic
JuPao1.connectionOutput = sm.interactable.connectionType.none
JuPao1.colorNormal = sm.color.new( 0xcb0a00ff )
JuPao1.colorHighlight = sm.color.new( 0xee0a00ff )
JuPao1.poseWeightCount = 1
JuPao1.fireDelay = 80 --ticks
JuPao1.minForce = 200
JuPao1.maxForce = 200
JuPao1.spreadDeg = 0.3
JuPao1.recoilForce = 0.05	--后坐力倍数
JuPao1.destructionRadius = 1.5	--破坏半径
JuPao1.shellMass = 15.0		--炮弹质量

function JuPao1.server_onCreate( self ) 
	self:server_init()
end

function JuPao1.server_init( self ) 
	self.fireDelayProgress = 0
	self.canFire = true
	self.parentActive = false
end

function JuPao1.server_onRefresh( self )
	self:server_init()
end

function JuPao1.server_onFixedUpdate( self, timeStep )
...
function JuPao1.server_tryFire( self )
	local parent = self.interactable:getSingleParent()
	if parent then
...
function JuPao1.client_onCreate( self )
	self.boltValue = 0.0
	self.bullets = {}
...
function JuPao1.client_onFixedUpdate( self, dt )
...
function JuPao1.client_onShoot( self, data )
	self.boltValue = 1.0
	local rot = sm.vec3.getRotation( sm.vec3.new( 1, 0, 0 ), self.shape.up )

	local position = self.shape.worldPosition + self.shape.up*0.2
	local bullet = {effect = sm.effect.createEffect("JuPao1Shell"), pos = position, direction = data.dir, alive = 0, grav = data.gravity} -- -  self.shape.up*0.5
