...
function DriverSeat.server_onRefresh( self )
	print("* * * * * REFRESH DriverSeat * * * * *")
end
...
function DriverSeat.sv_setGlobalFlag( self, props )
	if not self.interactable then return end
...
function DriverSeat.sv_getSeatInfo( self, player )
	local infoData = {}
...
function DriverSeat.client_canInteract( self, character )
	local interactKey = sm.gui.getKeyBinding("Use")
	local tinkerKey = sm.gui.getKeyBinding("Tinker")
	sm.gui.setInteractionText("", interactKey, "Use使用")
	sm.gui.setInteractionText("", tinkerKey, "Info连接信息")
...
function DriverSeat.client_onTinker( self, character, lookAt )
	if lookAt then
...
function DriverSeat.cl_openInfoGui( self, infoData )
...
function DriverSeat.client_onInteract( self, character, lookAt )
	if lookAt then
		self:cl_seat()
...
function DriverSeat.cl_seat( self )
...
function DriverSeat.cl_checkForReSeat( self )
...
function DriverSeat.client_onFixedUpdate( self, dt )
	if not self.interactable then return end
...
function DriverSeat.cl_setLogicFlags( self )
	...
	for k,child in pairs(self.interactable:getChildren()) do
		local cUuid = tostring(sm.shape.getShapeUuid(child:getShape()))
		if cUuid == "8003aacf-5494-11e6-beb8-9e71128cae77" or cUuid == "8004aacf-5494-11e6-beb8-9e71128cae77" then -- Seat Logic Breakout
