...
Cannon1 = class( nil )
Cannon1.maxParentCount = 1
Cannon1.maxChildCount = 0
Cannon1.connectionInput = sm.interactable.connectionType.logic
Cannon1.connectionOutput = sm.interactable.connectionType.none
Cannon1.colorNormal = sm.color.new( 0xcb0a00ff )
Cannon1.colorHighlight = sm.color.new( 0xee0a00ff )
Cannon1.poseWeightCount = 1
Cannon1.fireDelay = 80 --ticks
Cannon1.minForce = 400
Cannon1.maxForce = 400
Cannon1.spreadDeg = 0.3
Cannon1.recoilForce = 0.05	--后坐力倍数
Cannon1.destructionRadius = 1	--破坏半径
Cannon1.shellMass = 7.0		--炮弹质量

function Cannon1.server_onCreate( self ) 
	self:server_init()
end

function Cannon1.server_init( self ) 
	self.fireDelayProgress = 0
	self.canFire = true
	self.parentActive = false
end

function Cannon1.server_onRefresh( self )
	self:server_init()
end

function Cannon1.server_onFixedUpdate( self, timeStep )
...
function Cannon1.server_tryFire( self )
	local parent = self.interactable:getSingleParent()
	if parent then
...
function Cannon1.client_onCreate( self )
	self.boltValue = 0.0
	self.bullets = {}
...
function Cannon1.client_onFixedUpdate( self, dt )
...
function Cannon1.client_onShoot( self, data )
	self.boltValue = 1.0
