...
HangBaoDan = class()
HangBaoDan.maxParentCount = 1
HangBaoDan.maxChildCount = 0
HangBaoDan.connectionInput = sm.interactable.connectionType.power + sm.interactable.connectionType.logic
HangBaoDan.connectionOutput = sm.interactable.connectionType.power + sm.interactable.connectionType.logic
HangBaoDan.colorNormal = sm.color.new( 0xcb0a00ff )
HangBaoDan.colorHighlight = sm.color.new( 0xee0a00ff )
HangBaoDan.poseWeightCount = 1

HangBaoDan.fireDelay = 80 --ticks (2 seconds)
HangBaoDan.fuseDelay = 0.0625


--[[ Server ]]

-- (Event) Called upon creation on server
function HangBaoDan.server_onCreate( self )
	self:server_init()
end

-- (Event) Called when script is refreshed (in [-dev])
function HangBaoDan.server_onRefresh( self )
	self:server_init()
end

-- Initialize HangBaoDan
function HangBaoDan.server_init( self )
	self.alive = true
	self.counting = false
...
function HangBaoDan.server_onFixedUpdate( self, timeStep )
...
function HangBaoDan.server_tryExplode( self )
	if self.alive and self.shape.body.destructable then
		...
		sm.effect.playEffect("HangBaoDan",self.shape:getWorldPosition())
		sm.shape.destroyPart( self.shape )
	end
end

-- (Event) Called upon getting hit by a projectile.
function HangBaoDan.server_onProjectile( self, hitPos, hitTime, hitVelocity, hitType )
	if self.alive and self.shape.body.destructable then
		if self.counting then
...
function HangBaoDan.server_onMelee( self, hitPos, attackingCharacter, damage )
	if self.alive and self.shape.body.destructable then
		if self.counting then
			self.fireDelayProgress = self.fireDelayProgress + self.fireDelay * 0.5
		else
...
function HangBaoDan.server_onExplosion( self, center, destructionLevel )
...
function HangBaoDan.server_onCollision( self, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal )
...
function HangBaoDan.server_startCountdown( self )
	self.counting = true
...
function HangBaoDan.client_onCreate( self )
...
function HangBaoDan.client_hitActivation( self, hitPos )
...
function HangBaoDan.client_startCountdown( self )
