...
Explosive.connectionInput = sm.interactable.connectionType.power + sm.interactable.connectionType.logic
Explosive.connectionOutput = sm.interactable.connectionType.power + sm.interactable.connectionType.logic
Explosive.colorNormal = sm.color.new( 0xcb0a00ff )
Explosive.colorHighlight = sm.color.new( 0xee0a00ff )
...
function Explosive.server_onMelee( self, hitPos, attackingCharacter, damage )
	if self.alive and self.shape.body.destructable then
		if self.counting then
			self.fireDelayProgress = self.fireDelayProgress + self.fireDelay * 0.5
		else
