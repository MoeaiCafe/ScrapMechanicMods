dofile( "celldata.lua" )
dofile( "roads_and_cliffs.lua" )
dofile( "type_meadow.lua" )
dofile( "type_forest.lua" )
dofile( "type_field.lua" )
dofile( "type_burntForest.lua" )
dofile( "type_autumnForest.lua" )
dofile( "type_lake.lua" )
dofile( "type_mountain.lua" )
dofile( "type_desert.lua" )
dofile( "poi.lua" )
dofile( "start_area.lua" )
dofile( "../terrain_util.lua" )
...
function writeTile( tileId, xPos, yPos, size, rotation, terrainType )
	for y = 0, size - 1 do
		for x = 0, size - 1 do
			local cellX = x + xPos
			local cellY = y + yPos
			g_cellData.tileId[cellY][cellX] = tileId
			...
			if rotation == 1 then
				g_cellData.tileOffsetX[cellY][cellX] = y
				g_cellData.tileOffsetY[cellY][cellX] = ( size - 1 ) - x
			elseif rotation == 2 then
				g_cellData.tileOffsetX[cellY][cellX] = ( size - 1 ) - x
				g_cellData.tileOffsetY[cellY][cellX] = ( size - 1 ) - y
			elseif rotation == 3 then
				g_cellData.tileOffsetX[cellY][cellX] = ( size - 1 ) - y
				g_cellData.tileOffsetY[cellY][cellX] = x
			else
				g_cellData.tileOffsetX[cellY][cellX] = x
				g_cellData.tileOffsetY[cellY][cellX] = y
			end
		end
	end
end
...
function writePoi( poi )
	local variationNoise = poi.variation and poi.variation - 1 or sm.noise.intNoise2d( poi.x, poi.y, g_cellData.seed + 2854 )
	local rotationNoise = poi.rotation or sm.noise.intNoise2d( poi.x, poi.y, g_cellData.seed + 9439 )
	local tileId, rotation = getPoiTileIdAndRotation( poi.type, variationNoise, rotationNoise )
...
function evaluateRoadsAndCliffs()
	...
	for cellY = g_cellData.bounds.yMin, g_cellData.bounds.yMax do
		for cellX = g_cellData.bounds.xMin, g_cellData.bounds.xMax do
			...
			if tileId ~= 0 and g_cellData.tileId[cellY][cellX] == 0 then
				g_cellData.tileId[cellY][cellX] = tileId
				g_cellData.rotation[cellY][cellX] = rotation
...
function evaluateType( type, fn )
	forEveryCell( function( x, y )
		...
		if tileId ~= 0 and g_cellData.tileId[y][x] == 0 then
			g_cellData.tileId[y][x] = tileId
			g_cellData.rotation[y][x] = rotation
			g_cellData.tileOffsetX[y][x] = 0
			g_cellData.tileOffsetY[y][x] = 0
