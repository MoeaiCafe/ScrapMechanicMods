...
local ExI_Toggles = {
	...
	foodGained = { onOffName = "FoodGained", vis = {}, special = {} }
}
...
local FoodRecoveryThreshold = 1 -- 饱值度高于多少时恢复血量
local FastFoodRecoveryThreshold = 5 -- 饱值度高于多少时快速恢复血量
local HpRecovery = 100 * PerMinute -- 每分钟回血速度
local FastHpRecovery = 150 * PerMinute -- 快速每分钟回血速度
local FoodCostPerHpRecovery = 0.02 -- 每回1滴血消耗的饱值度
local FastFoodCostPerHpRecovery = 0.002 -- 快速每回1滴血消耗的饱值度

local FoodCostPerStamina = 0.002 -- 耐力恢复饱值度消耗
local WaterCostPerStamina = 0.01 -- 耐力恢复口渴值消耗
local SprintStaminaCost = 0.7 / 40 -- 每Tick跑动对耐力消耗
local CarryStaminaCost = 1.4 / 40 -- 每Tick负重对耐力消耗

local FoodLostPerSecond = 100 / 10 / 24 / 60 -- 每秒消耗的饱值度
local WaterLostPerSecond = 100 / 10 / 24 / 60 -- 每秒消耗的口渴值

local BreathLostPerTick = ( 100 / 100 ) / 40 -- 游泳时每Tick消耗的氧气
...
function SurvivalPlayer.server_onCreate( self )
	...
	if self.sv.saved.exi == nil then
		if self.sv.saved then
			...
			self.sv.saved.exi.foodGained = true

			self.sv.saved.exi.saveVersion = 3
			--[[
			Save version 0:
	...
	elseif self.sv.saved.exi.saveVersion ~= 3 then
		print("Updating Expanded info save version to version 3...")
		...
		if saveexi.saveVersion <= 2 then
			saveexi.foodGained = true
		end

		saveexi.saveVersion = 3
		self.storage:save( self.sv.saved )
	end
	--to here
...
function SurvivalPlayer.client_onCreate( self )
	...
	if self.player == sm.localPlayer.getPlayer() then
		...
		ExI_CtrlpanelGUI:setButtonCallback( "FoodGainedOn", "client_exiConButton" )
		ExI_CtrlpanelGUI:setButtonCallback( "FoodGainedOff", "client_exiConButton" )
...
function SurvivalPlayer.client_onClientDataUpdate( self, data )
	if sm.localPlayer.getPlayer() == self.player then
		...
		if not fullyLoaded then
			...
			self:client_OnOffButton("FoodGained", self.cl.exi.foodGained)
...
function SurvivalPlayer.cl_localPlayerUpdate( self, dt )
	...
	if character then
		...
		--Food/thirst replenished animation 1/2
		if foodGained.visible then
			if foodGained.sliderRendPosFood ~= foodGained.sliderPosFood then
				foodGained.sliderRendPosFood = foodGained.sliderRendPosFood-(foodGained.sliderRendPosFood-foodGained.sliderPosFood)*dt*5
				for i=-1,1 do
					g_survivalHud:setItemIcon( "FoodGained".. tostring(i+mfloor(foodGained.anim_keyframe)), "ExI_GuiMap", "ExI_Bar_Orange", tostring(mfloor(foodGained.sliderRendPosFood)))
				end
				if round(foodGained.sliderRendPosFood) == foodGained.sliderPosFood then
					foodGained.sliderRendPosFood = foodGained.sliderPosFood
					for i=1,32 do
						g_survivalHud:setItemIcon( "FoodGained".. tostring(i), "ExI_GuiMap", "ExI_Bar_Orange", tostring(mfloor(foodGained.sliderPosFood)))
					end
				end
			end
			if foodGained.sliderRendPosThirst ~= foodGained.sliderPosThirst then
				foodGained.sliderRendPosThirst = foodGained.sliderRendPosThirst-(foodGained.sliderRendPosThirst-foodGained.sliderPosThirst)*dt*5
				for i=-1,1 do
					g_survivalHud:setItemIcon( "ThirstGained".. tostring(i+mfloor(foodGained.anim_keyframe)), "ExI_GuiMap", "ExI_Bar_Blue", tostring(mfloor(foodGained.sliderRendPosThirst)))
				end
				if round(foodGained.sliderRendPosThirst) == foodGained.sliderPosThirst then
					foodGained.sliderRendPosThirst = foodGained.sliderPosThirst
					for i=1,32 do
						g_survivalHud:setItemIcon( "ThirstGained".. tostring(i), "ExI_GuiMap", "ExI_Bar_Blue", tostring(mfloor(foodGained.sliderPosThirst)))
					end
				end
			end
		end
		...
		if countwait >= 2 then
			...
			--Better unauthorized farming detected message (FOR FUTURE UPDATE if this doesn't get completely scrapped)
			--[[if true then
			...
			if self.cl.exi.DamageIndicator == true then
				...
				if hit and result.type == "character" then
					...
					if Indicator_StayVis <= 0 and lookingAtCharacter ~= nil then
			...
			if self.cl.exi.clock == true then
				...
				--g_survivalHud:setText("Time", tostring(hour).. ":".. tostring(mfloor(((sm.game.getTimeOfDay()*24)-hour)*6)*10))
			...
			if self.cl.exi.foodGained then
				local edible = sm.item.getEdible(hotbaritem)

				-- food/thirst gained indicator

				if edible then
					if foodGained.visible == false then
						foodGained.sliderRendPosFood = mfloor((self.cl.stats.food/self.cl.stats.maxfood)*120)
						foodGained.sliderRendPosThirst = mfloor((self.cl.stats.water/self.cl.stats.maxwater)*120)
					end

					local lastKF = mfloor(foodGained.anim_keyframe)

					if foodGained.anim_in then
						foodGained.anim_keyframe = foodGained.anim_keyframe+dt*40
						if foodGained.anim_keyframe > 30 then foodGained.anim_in = false end
					else
						foodGained.anim_keyframe = foodGained.anim_keyframe-dt*40
						if foodGained.anim_keyframe < 10 then foodGained.anim_in = true end
					end

					if lastKF ~= mfloor(foodGained.anim_keyframe) then
						foodGained.sliderPosFood = mfloor(min(1, (self.cl.stats.food+edible.foodGain)/self.cl.stats.maxfood)*120)
						foodGained.sliderPosThirst = mfloor(min(1, (self.cl.stats.water+edible.waterGain)/self.cl.stats.maxwater)*120)
						foodGained.visible = true

						local newKF = tostring(mfloor(foodGained.anim_keyframe))
						g_survivalHud:setVisible("FoodGained".. tostring(lastKF), false)
						g_survivalHud:setVisible("FoodGained".. newKF, true)
						g_survivalHud:setVisible("ThirstGained".. tostring(lastKF), false)
						g_survivalHud:setVisible("ThirstGained".. newKF, true)
					end

				elseif foodGained.visible then
					local lastKF = mfloor(foodGained.anim_keyframe)
					foodGained.anim_keyframe = foodGained.anim_keyframe-dt*40
					if lastKF ~= mfloor(foodGained.anim_keyframe) then
						local newKF = tostring(mfloor(foodGained.anim_keyframe))
					...
					if foodGained.anim_keyframe < 0 then
						foodGained.visible, foodGained.anim_in, foodGained.sliderRendPosFood, foodGained.sliderRendPosThirst, foodGained.anim_keyframe = false, true, 0, 0, 0
					end
...
function SurvivalPlayer.client_togglesomestuff( self, action, dropdownState )
	...
	elseif action == "DynamicRaidBG" then
		self:client_raidguicmd("t", nil, false)
		self:client_raidguicmd("t", nil, false)
	elseif action == "DamageIndicator" and newstate == false and lookingAtCharacter ~= nil then
		lookingAtCharacter = nil
		self.network:sendToServer("sv_updatePlrThing", nil)
	end
...
function SurvivalPlayer.client_exiConButton( self, buttonName )
	...
	elseif buttonName == "PopUpYNYes" then
		...
		if not self.cl.exi.foodGained then self:client_togglesomestuff("foodGained", nil) end
...
function SurvivalPlayer.sv_unitUpdates( self, data )
	...
	for k in pairs(sv_lookingAtCharacter) do
		if sv_lookingAtCharacter[k].charId == id then
			if sv_lookingAtCharacter[k].data[2] ~= data[2] or sv_lookingAtCharacter[k].data[4] ~= data[4] then
